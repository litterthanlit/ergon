...
export function TemplateSwitcher({ activeId, onSelect }: Props) {
  return (
    <div className="flex items-center gap-1.5 px-4 py-3 bg-white/90 backdrop-blur-md border-t border-ergon-border">
      <span className="text-[11px] text-ergon-muted uppercase tracking-[0.15em] font-medium mr-2 shrink-0">
        Templates
      </span>
      {templates.map((t) => (
        <button
          ...
          className={`px-3 py-1.5 text-xs rounded transition-all duration-150 shrink-0 ${
            activeId === t.id
